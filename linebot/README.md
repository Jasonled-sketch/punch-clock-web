...
## 誰看得到什麼（2026/08/26 定案）

「下班後回覆」不公開、不列入考核分數，只有老闆看得到，累積起來當**年終參考**。
這樣既留得住紀錄，也避開「下班後被要求回訊息＝工時」的爭議。

| 看的人 | 看得到 |
|---|---|
| `MSGKPI_DETAIL_EMAILS` 名單內（預設只有 Jason） | 全部：回覆率、上班內／下班後拆分、明確／推定、回覆速度、群裡發言數、逐則誰沒回、🔒 下班後回覆排行、發公告按鈕 |
| 其他管理者（hr／主管） | **只有回覆率**：團隊回覆率、每日趨勢、個人回覆率排行（含需回／已回則數） |

**這是後端擋的，不是前端藏的**——非名單身分呼叫 API，回應裡根本沒有 `acked_off`、
`median_lag`、`spoke`、訊息內容、未回名單這些欄位（`detail: false`）。
前端看到 `detail:false` 就自動縮成精簡版。改前端擋不住，也偷不到。

呼叫者身分從 `?email=` 來（Cloudflare Worker 驗完 Google 登入後注入，與 `/calc/punch-edit`
等路由同一套）。**所以「只看某一個人」的篩選參數叫 `who`，不能叫 `email`**，同名會被身分覆蓋。
...
| `MSGKPI_DETAIL_EMAILS` | `covernself@gmail.com` | 誰看得到明細（下班後拆分／速度／發言數／逐則未回）。逗號分隔，其他人只看得到回覆率 |
...
```
14 個案例涵蓋：群組隔離、四種錨點判定、三種回應來源、上班/下班切分、
重複回應去重、每日結算數字、重跑結算不重複灌、API 彙總與未回名單、
明細權限擋不擋得住（非名單身分拿不到下班後欄位）、who 篩選參數。
...
- **隱私**：預設會存群組訊息前 300 字（錨點 500 字）供查核。不想存就設 `MSGKPI_STORE_TEXT=0`，
  統計照常，只是頁面上看不到訊息內容。
- **勞基法**：下班後回覆**不列入公開考核分數**，只進老闆私人的年終參考區（Jason 2026/08/26 定案）。
  這是刻意的：台灣實務上把下班後回訊息綁進考核，容易被主張為工時／加班。
  現在的做法是「照樣記錄、不公開、不計分」，要當年終獎金的加分項由老闆自己拿捏。
  資料本身照樣分開存（`acked_on` / `acked_off`），未來政策要改隨時調得動。
